...
class ServiceSpoofer(ScapyServer, Publisher):
    ...
    def _startIpTables(self):
        ...
        for service in self.services:
            if service.tcp:
                set_iptable = iptables + ' -I OUTPUT -p tcp --tcp-flags RST RST --sport ' + str(
                    service.port) + ' -j DROP'
            elif service.udp:
    ...
    def answerTCP(self, packet):
        try:
            ...
            if self.subscribers and victim_ip in self.subscribers[0].white_list or victim_ip in self.subscribers[
                0].black_list:
                return
...
class Service:
    ...
    def findExpression(service_id):
        ...
        # path = "OhHoneypot/OhHoneyPy/" if SessionManager.getInstance().is_android else ''
